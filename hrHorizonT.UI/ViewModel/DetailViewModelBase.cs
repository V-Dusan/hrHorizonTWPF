    ...
    public abstract class DetailViewModelBase : ViewModelBase, IDetailViewModel
    {
        ...
        protected async virtual void OnCloseDetailViewExecute()
        {
            ...
            if (HasChanges)
            {
                var result = await MessageDialogService.ShowOkCancelDialogAsync(
                    "Napravili ste promene. Da li želite da odustanete od promena?", "Pitanje?");
        ...
        protected async Task SaveWithOptimisticConcurrencyAsync(Func<Task> saveFunc, Action afterSaveAction)
        {
            ...
            catch (DbUpdateConcurrencyException ex)
            {
                ...
                {
                    await MessageDialogService.ShowInfoDialogAsync("Model je obrisan od strane drugog korisnika");
                    RaiseDetailDeletedEvent(Id);
                    return;
                }

                var result = await MessageDialogService.ShowOkCancelDialogAsync("Neko od korisnika je promenio model u međuvremenu." +
                    " Pritisnite OK da snimite Vaše promene, pritisnite Cancel da vratite podatke na početno stanje.", "Pitanje?");
